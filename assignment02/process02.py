...
class Basket :
    ...
    def use_eggs(self, index) :
        print(f"{ctime()} Kitchen-{index} : Chef-{index} has lock with eggs reamaining {self.eggs}")
        self.eggs -= 1
        print(f"{ctime()} Kitchen-{index} : Chef-{index} has released with eggs reamaining {self.eggs}")
